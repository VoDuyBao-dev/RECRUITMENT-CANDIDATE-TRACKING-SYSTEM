...
public class HRInterviewController {
        ...
        public ApiResponse<PageResponse<InterviewResponse>> getInterviewsByApplication(
                ...
                PageResponse<InterviewResponse> interviews = interviewService.getInterviewsByApplication(applicationId,
                                page, size);

                return ApiResponse.<PageResponse<InterviewResponse>>builder()
                                .code(HttpStatus.OK.value())
                                .message(String.format("Retrieved %d interviews successfully",
                                                interviews.getItems().size()))
                                .result(interviews)
                                .build();
        }
        ...
        public ApiResponse<PageResponse<EvaluationResponse>> getEvaluationsByInterview(@PathVariable Long id,
                ...
                PageResponse<EvaluationResponse> evaluations = interviewService.getEvaluationsByInterview(id, page,
                                size);

                return ApiResponse.<PageResponse<EvaluationResponse>>builder()
                                .code(HttpStatus.OK.value())
                                .message(String.format("Retrieved %d evaluations successfully",
                                                evaluations.getItems().size()))
                                .result(evaluations)
                                .build();
        }

        // Lấy đánh giá của các ứng viên đã được TẤT CẢ interviewer đánh giá xong
        @GetMapping("/{id}/evaluations/completed")
        public ApiResponse<PageResponse<EvaluationResponse>> getCompletedEvaluationsByInterview(
                        @PathVariable Long id,
                        @RequestParam(defaultValue = "0") int page,
                        @RequestParam(defaultValue = "10") int size) {

                PageResponse<EvaluationResponse> evaluations = interviewService.getCompletedEvaluationsByInterview(id,
                                page, size);

                return ApiResponse.<PageResponse<EvaluationResponse>>builder()
                                .code(HttpStatus.OK.value())
                                .message("Retrieved completed evaluations successfully")
                                .result(evaluations)
                                .build();
        }

        // danh sách người phỏng vấn cho một cuộc phỏng vấn
        // @GetMapping("/{id}/interviewers")
        // public ApiResponse<List<String>> getInterviewersByInterview(@PathVariable
        // Long id) {
        // List<String> interviewers = interviewService.getInterviewersByInterview(id);
        // return ApiResponse.<List<String>>builder()
        // .code(HttpStatus.OK.value())
        // .message("Retrieved interviewers successfully")
        // .result(interviewers)
        // .build();
        // }

        // danh sách tất cả người phỏng vấn
        @GetMapping("/interviewers/all")
        public ApiResponse<List<InterviewersResponse>> getAllInterviewers() {

                List<InterviewersResponse> interviewers = interviewService.getAllInterviewers();

                return ApiResponse.<List<InterviewersResponse>>builder()
                                .code(HttpStatus.OK.value())
                                .message("Retrieved all interviewers successfully")
                                .result(interviewers)
                                .build();
        }
